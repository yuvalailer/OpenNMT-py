...
class SiameseDiscriminator(nn.Module):
    def __init__(self, decoder_dim, hidden_dim, bi=False, num_layers=1, batch_first=False, dropout=0.2):
        ...
        self.bn2 = nn.BatchNorm1d(hidden_dim*(2 if bi else 1))
        self.fc = nn.Linear(hidden_dim*(2 if bi else 1), 1)

    def forward(self, dec_output, enc_output):
        dec = self.seq2vec(dec_output)
        enc = self.seq2vec(enc_output)
        fc_input = self.bn2(torch.cat([enc, dec], dim=-1))
    ...
    def seq2vec(self, x):
        ...
        if self.bi:
            last_tensor = torch.cat([packed_output[:,:,0], packed_output[:,:,-1]], dim=-1)
        else:
            last_tensor = packed_output[:,:,-1]
        return last_tensor
